 ...
 */
 (function($){
     ...
     */
        function _markupContent(tag, html, index) {
            ...
            $outer.children().each(function (i, element) {
                element.classList.add('line', 'line-index-' + index);
            });
     ...
     * options
     */
        $.fn.splitLines = function(options, action) {
            var settings = {
                width: 'auto',
                tag: '<div>',
                wrap: '',
            ...
            for (var index=0; index<arr.length; index++) {
                ...
                // Nếu action là "destroy", khôi phục nội dung gốc
                if (action === 'destroy') {
                    var originalContent = tmpElement.data('originalContent');
                    if (originalContent) {
                        tmpElement.html(originalContent);
                    }
                    continue;
                }
    
                // Lưu nội dung gốc nếu chưa lưu
                if (!tmpElement.data('originalContent')) {
                    tmpElement.data('originalContent', tmpElement.html());
                }
